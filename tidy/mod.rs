...
static TEMP_DIRECTORIES: phf::Set<&'static str> = phf_set! {
  "__MACOSX",
};

static TEMP_FILES: phf::Set<&'static str> = phf_set! {
  "thumbs.db",
};
...
pub(crate) struct Args {
    ...
    /// If false, will perform the delete based on 
...
pub(crate) fn run(args: &Args, verbose: &Verbosity) -> Result<()> {
    ...
    let duplicate_thread = std::thread::spawn(move || {
        tidy_file_system(&args, &pb_detail, path_rx);
    });
...
fn tidy_file_system(args: &Args, pb_detail: &ProgressBar, path_rx: Receiver<FileMetadata>) {
